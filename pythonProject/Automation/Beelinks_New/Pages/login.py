...
class llogin():
    ...
    def forgot_passwords(self):
        ...
        f = 0
        while f < size_email:
            self.driver.find_element_by_xpath(self.emailof_forgot_password).send_keys(email_fotgot[
                                                                                          f])  # entee thg  email if email is invalid then it checks the message  and if message is present  then it will try new email
            try:
